  ...
  <div class="md:ml-64 min-h-screen flex flex-col">
    ...
    <header class="bg-white gradient-to-r from-pink-500 to-yellow-400 text-black shadow-md py-4 px-6 sticky top-0 z-30 md:ml-0">
      <div class="flex justify-between items-center">
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h1 class="text-xl md:text-2xl font-bold">Purchase Orders</h1>
            <p class="text-black text-xs md:text-sm mt-1">All purchase orders with search & pagination</p>
          </div>
        </div>
    ...
    <main class="flex-1 p-6 space-y-6 md:ml-0">
      ...
      <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100">
        <form method="GET" action="{{ route('purchase-orders.index') }}" class="flex flex-col sm:flex-row gap-4">
          <div class="flex-1">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by PO Number, Customer, Product, or SKU..." class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-green focus:border-transparent" />
          </div>
          <button type="submit" class="px-6 py-2.5 bg-brand-green text-white rounded-lg hover:bg-green-600 transition shadow-sm hover:shadow-md font-medium">
            Search
          </button>
          @if(request('search'))
            <a href="{{ route('purchase-orders.index') }}" class="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition font-medium">
              Clear
            </a>
          @endif
        </form>
      ...
      <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <div>
            <h2 class="text-xl font-semibold text-gray-800">All Purchase Orders</h2>
            <p class="text-sm text-gray-500">Total: {{ $purchaseOrders->total() }} orders | Showing {{ $purchaseOrders->firstItem() ?? 0 }}-{{ $purchaseOrders->lastItem() ?? 0 }} of {{ $purchaseOrders->total() }}</p>
          </div>
        ...
        @if($purchaseOrders->isEmpty())
          <p class="text-sm text-gray-500 py-6 text-center">No purchase orders found. @if(request('search'))Try a different search term.@endif</p>
        @else
          <!-- Mobile Card View -->
            ...
            @foreach($purchaseOrders as $po)
              ...
              <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition">
                <div class="flex justify-between items-start mb-3">
                  <div>
                    <h3 class="font-semibold text-gray-900 text-lg">{{ $po->po_number ?? 'N/A' }}</h3>
                    <p class="text-sm text-gray-600 mt-1">{{ $po->customer_name ?? 'N/A' }}</p>
                  </div>
                  <span class="inline-flex items-center px-3 py-1 text-xs rounded-full {{ $chipBg }} {{ $chipText }} font-medium">{{ $label }}</span>
                </div>
                
                <div class="space-y-2 border-t border-gray-100 pt-3">
                  <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Product:</span>
                    <span class="text-sm font-medium text-gray-900">{{ $po->product_name ?? 'N/A' }}</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">SKU:</span>
                    <span class="text-sm text-gray-900">{{ $po->sku_number ?? 'N/A' }}</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Quantity:</span>
                    <span class="text-sm font-medium text-gray-900">{{ number_format($po->quantity ?? 0) }}</span>
                  </div>
                  <div class="flex justify-between items-center">
                    <span class="text-sm text-gray-600">Unit Price:</span>
                    <span class="text-sm text-gray-900">₹{{ number_format($po->product_price ?? 0, 2) }}</span>
                  </div>
                  <div class="flex justify-between items-center pt-2 border-t border-gray-100">
                    <span class="text-sm font-semibold text-gray-700">Total Amount:</span>
                    <span class="text-base font-bold text-brand-green">₹{{ number_format($po->total_amount ?? 0, 2) }}</span>
                  </div>
                  <div class="flex justify-between items-center pt-2 border-t border-gray-100">
                    <div>
                    ...
                    <div class="text-right">
                      ...
                      <p class="text-sm text-gray-900">{{ $po->delivery_date ? $po->delivery_date->format('M d, Y') : 'N/A' }}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <tbody class="text-gray-700">
                @foreach($purchaseOrders as $po)
                  ...
                  <tr class="hover:bg-gray-50 transition">
                    <td class="py-3 px-4 border-b border-gray-100 font-medium text-gray-900">{{ $po->po_number ?? 'N/A' }}</td>
                    <td class="py-3 px-4 border-b border-gray-100">{{ $po->customer_name ?? 'N/A' }}</td>
                    <td class="py-3 px-4 border-b border-gray-100">{{ $po->product_name ?? 'N/A' }}</td>
                    <td class="py-3 px-4 border-b border-gray-100 text-sm text-gray-600">{{ $po->sku_number ?? 'N/A' }}</td>
                    <td class="py-3 px-4 border-b border-gray-100">{{ number_format($po->quantity ?? 0) }}</td>
                    <td class="py-3 px-4 border-b border-gray-100">₹{{ number_format($po->product_price ?? 0, 2) }}</td>
                    <td class="py-3 px-4 border-b border-gray-100 font-semibold text-gray-900">₹{{ number_format($po->total_amount ?? 0, 2) }}</td>
                    <td class="py-3 px-4 border-b border-gray-100 text-sm text-gray-600">
                      @if($po->created_at)
                        <div>{{ $po->created_at->format('M d, Y') }}</div>
                        <div class="text-xs text-gray-500">{{ $po->created_at->format('h:i A') }}</div>
                      @else
                        N/A
                      @endif
                    </td>
                    <td class="py-3 px-4 border-b border-gray-100">{{ $po->delivery_date ? $po->delivery_date->format('M d, Y') : 'N/A' }}</td>
